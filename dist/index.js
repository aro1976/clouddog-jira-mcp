...
// ── Projects ──
server.tool("list_projects", "List all Jira projects", { maxResults: z.number().optional(), startAt: z.number().optional() }, async ({ maxResults, startAt }) => {
...
server.tool("get_project", "Get a Jira project by key or ID", { projectIdOrKey: z.string() }, async ({ projectIdOrKey }) => {
...
server.tool("get_issue", "Get a Jira issue by key or ID", {
...
server.tool("edit_issue", "Edit an existing Jira issue", {
    issueIdOrKey: z.string(),
...
server.tool("delete_issue", "Delete a Jira issue", { issueIdOrKey: z.string(), deleteSubtasks: z.boolean().optional() }, async ({ issueIdOrKey, deleteSubtasks }) => {
...
server.tool("assign_issue", "Assign a Jira issue to a user", { issueIdOrKey: z.string(), accountId: z.string().nullable() }, async ({ issueIdOrKey, accountId }) => {
...
server.tool("get_transitions", "Get available transitions for an issue", { issueIdOrKey: z.string() }, async ({ issueIdOrKey }) => {
    const data = await jiraRequest(`/issue/${issueIdOrKey}/transitions`);
    return { content: [{ type: "text", text: JSON.stringify(data, null, 2) }] };
});
server.tool("transition_issue", "Transition an issue to a new status", {
    issueIdOrKey: z.string(),
...
// ── Comments ──
server.tool("get_comments", "Get comments for an issue", { issueIdOrKey: z.string(), maxResults: z.number().optional(), startAt: z.number().optional() }, async ({ issueIdOrKey, maxResults, startAt }) => {
    const params = new URLSearchParams();
    if (maxResults)
        params.set("maxResults", String(maxResults));
    if (startAt)
...
server.tool("add_comment", "Add a comment to an issue", { issueIdOrKey: z.string(), body: z.string() }, async ({ issueIdOrKey, body: commentBody }) => {
...
// ── Worklogs ──
server.tool("get_worklogs", "Get worklogs for an issue", { issueIdOrKey: z.string(), maxResults: z.number().optional(), startAt: z.number().optional() }, async ({ issueIdOrKey, maxResults, startAt }) => {
    const params = new URLSearchParams();
    if (maxResults)
        params.set("maxResults", String(maxResults));
    if (startAt)
...
server.tool("add_worklog", "Add a worklog entry to an issue (log time spent)", {
    issueIdOrKey: z.string(),
...
server.tool("update_worklog", "Update an existing worklog entry", {
    issueIdOrKey: z.string(),
    worklogId: z.string(),
...
server.tool("delete_worklog", "Delete a worklog entry", { issueIdOrKey: z.string(), worklogId: z.string() }, async ({ issueIdOrKey, worklogId }) => {
